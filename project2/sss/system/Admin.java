...
public class Admin extends User implements Serializable {
	...
	public void addUser(String username,String password) throws NumberFormatException, IOException, ParseException {
		...
		if(choice==1) {
			...
			if(choice==2){
		    	...
		    	u=new Mananger(ID, password,username,name,surname,birthDate,email,language,gender,salary,manangers);
			}
			else if(choice==3){
				...
				u=new SupportKBTU(ID, password,username, name, surname, birthDate, email, language, gender, salary);
		    	...
		    	if(choice==5){
					...
					System.out.println("GraduateStudentType:1-Master\n2-PhD");
			    	int choiceType=Integer.parseInt(bReader.readLine());
			    	GradStudents type;
			    	if(choiceType==1){
			    		type=GradStudents.Master;
			    	}else {
			    		type=GradStudents.PhD;
			    	}
			    	u=new GraduateStudent(ID, password,username, name, surname, birthDate, email, language, gender, salary, studyYear, schools,type);
		    	}
			}
			else{
    ...
    public void seeAllUsers(){
        ...
        for(User u:users){
        	if(u instanceof Admin){
        	...
        	}else if(u instanceof Mananger){
        		Mananger newu=(Mananger) u;
        		System.out.println(newu.toString());
        		...
        		GraduateStudent newu=(GraduateStudent) u;
        		System.out.println(newu.toString());
        	}else if(u instanceof SupportKBTU){
        		SupportKBTU newu=(SupportKBTU) u;
        		System.out.println(newu.toString());
